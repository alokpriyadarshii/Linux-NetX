...
# Linux-Net 0.4.3 镜像离线分发脚本 (支持初始化部署与代码更新)
...
# Linux-Net 业务镜像
APP_IMAGES=(
    "linux-net-controller"
    "linux-net-node-worker"
    "linux-net-fifo-worker"
    "linux-net-archiver-worker"
)
...
TAR_FILE="linux-net-images-$VERSION-$MODE.tar"
...
if [[ "$*" == *"--restart"* ]]; then
    ...
    kubectl rollout restart deployment/linux-net-controller -n linux-net
    kubectl rollout restart deployment/linux-net-node-worker -n linux-net
    kubectl rollout restart deployment/linux-net-fifo-worker -n linux-net
    echo "等待更新完成..."
    kubectl rollout status deployment/linux-net-controller -n linux-net
    echo "✅ 系统已平滑更新至最新镜像版本。"
else
    echo "提示: 如需应用代码更新，请手动执行 'kubectl rollout restart deployment -n linux-net' 或在脚本后追加 --restart 参数。"
fi
