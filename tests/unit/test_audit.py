...
from linux_net.services.audit import rpc_audit_callback
from linux_net.worker.archiver import (
    _ensure_ttl_index,
...
def test_rpc_audit_callback_enqueues(monkeypatch, unit_runtime):
    ...
    # import is stale after unit_runtime reloads linux_net.services.rediz.
    ...
    assert len(jobs) == 1
    job = jobs[0]
    assert job.func_name == "linux_net.worker.archiver.process_audit_log"
