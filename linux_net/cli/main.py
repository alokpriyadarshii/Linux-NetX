...
from linux_net.models import DriverConnectionArgs, DriverName, QueueStrategy
...
from linux_net.models.response import (
    BatchSubmitJobResponse,
    JobInResponse,
)
from linux_net.plugins.drivers.napalm.model import NapalmExecutionRequest
from linux_net.plugins.drivers.netmiko.model import NetmikoExecutionRequest
from linux_net.plugins.drivers.paramiko.model import ParamikoExecutionRequest
from linux_net.plugins.drivers.pyeapi.model import PyeapiExecutionRequest
...
client: Optional["LinuxNetClient"] = None
...
def ensure_client() -> "LinuxNetClient":
    if client is None:
        raise RuntimeError("LinuxNetClient is not initialized")
    return client
...
def ensure_context() -> tuple["LinuxNetClient", "Printer", "RootSettings"]:
...
class RootSettings(BaseSettings):
    ...
    # API config
    endpoint: HttpUrl = Field(HttpUrl(DEFAULT_ENDPOINT), description="Linux-Net API endpoint URL")
    ...
    def cli_cmd(self) -> None:
        ...
        config = self
        printer = Printer()
        client = LinuxNetClient(str(config.endpoint), config.api_key)
...
class Printer:
    ...
    def __init__(self):
        ...
        self.log = logging.getLogger("linux_net-cli")
...
class LinuxNetClient:
    def __init__(
        ...
        Args:
            endpoint: Linux-Net API endpoint URL
