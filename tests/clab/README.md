...
- `ssh1`：`lscr.io/linuxserver/openssh-server:latest`，管理地址 `172.20.20.21`，
  用户名/密码 `linux_net/linux_net`（可通过 env 覆盖），监听端口 **2222**（`E2E_SSH_*`）。
- `redis1`：`redis:7-alpine`，管理地址 `172.20.20.30`，无密码，已关闭 protected-mode，绑定 0.0.0.0（`E2E_REDIS_*` 可覆盖）。
- 管理网络：`linux_net-e2e`，前缀 `172.20.20.0/24`。容器的管理口可直接从宿主访问，无需端口映射。
...
- 启动示例：
  ...
  # 如遇残留网络导致部署失败，可手动清理：
  docker network rm linux_net-e2e 2>/dev/null || true
  ```
