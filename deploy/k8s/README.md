# Linux-Net Kubernetes 生产级 HA 部署指南

本目录包含 Linux-Net 在 Kubernetes 环境下的生产级部署资源清单。
...
```
> **注意**: 部署后需等待几分钟，直到 `linux-net-redis-0`, `1`, `2` 完成持久卷绑定、镜像拉取（`quay.io/opstree/redis:v7.0.12`）并运行。
...
kubectl logs job/vault-init -n linux-net
```
> 💡 **断电恢复特性**: 由于该任务具备完全幂等性。如果日后整个 K8s 物理集群关机重启，导致 Vault 节点集体进入 `Sealed` (锁定) 状态，你只需**重新 apply 执行一次该任务**（`kubectl delete job vault-init -n linux-net && kubectl apply -f deploy/k8s/05-vault-init.yaml`），它就能自动从 Secret 读取密钥并帮所有节点无痛解封。

### [Step 5] 部署 Linux-Net 业务控制面与数据面
...
# 3. 开启业务审计状态
kubectl patch configmap linux-net-config -n linux-net --type merge -p '{"data":{"LINUX_NET_MONGODB__ENABLED":"true"}}'
kubectl rollout restart deployment/linux-net-archiver-worker -n linux-net
```
```
...
### 2. 集群状态巡检命令
- **全栈概览**：`kubectl get pods,pvc,ingress,redisreplication -n linux-net`
- **监控应用日志**：`kubectl logs -f deployment/linux-net-controller -n linux-net`
- **查看 Vault 选主状态**：
  ```bash
  kubectl exec vault-0 -n linux-net -- sh -c "VAULT_TOKEN=1Jj1OB3RRsXLygXPzTwMn6KG vault operator raft list-peers"
  ```
...
| 凭据名称 | 作用范围 | 权限等级 | 获取命令 |
| :--- | :--- | :--- | :--- |
| **Vault Initial Root Token** | **Vault 管理**：登录 UI、管理策略、手动调试 Vault 核心 | **最高 (Supreme)** | `kubectl get secret vault-unseal-key -n linux-net -o jsonpath='{.data.root-token}' \| base64 -d && echo` |
| **Vault Operational Token** | **业务访问**：用于 Linux-Net 程序内部读写 KV 密钥 | **高 (Root级/隔离)** | `kubectl get secret linux-net-secrets -n linux-net -o jsonpath='{.data.vault-token}' \| base64 -d && echo` |
| **Linux-Net API Key** | **SDK/客户端**：用于在请求 API 时通过 `X-API-KEY` 进行头部鉴权 | **应用级 (App)** | `kubectl get secret linux-net-secrets -n linux-net -o jsonpath='{.data.api-key}' \| base64 -d && echo` |

#### 🔑 运维备忘录：
- **解封密钥 (Unseal Key)**：用于 Vault 集群崩溃或重启后的手动/自动解锁，存储在 K8s Secret 中：
  `kubectl get secret vault-unseal-key -n linux-net -o jsonpath='{.data.unseal-key}' | base64 -d && echo`
- **Redis 密码**：
  `kubectl get secret linux-net-secrets -n linux-net -o jsonpath='{.data.password}' | base64 -d && echo`
