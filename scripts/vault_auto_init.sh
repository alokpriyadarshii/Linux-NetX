#!/bin/sh
# Linux-Net Vault Auto-Initialization and Guardian Script
...
while true; do
    ...
    if vault status 2>&1 | grep -q "Sealed.*false"; then
        ...
        if [ -f /vault/data/root.token ]; then
            ...
            # Align .env token
            if [ -n "$LINUX_NET_VAULT_TOKEN" ]; then
                # Check if current token in .env is already valid
                if ! VAULT_TOKEN="$LINUX_NET_VAULT_TOKEN" vault token lookup > /dev/null 2>&1; then
                    echo "Target token in .env is invalid or not yet aligned. Aligning..."
                    # Note: ID cannot start with 'hvs.' prefix
                    if VAULT_TOKEN="$RT" vault token create -id="$LINUX_NET_VAULT_TOKEN" -policy="root" > /dev/null 2>&1; then
                        echo "Token alignment successful: $LINUX_NET_VAULT_TOKEN"
                    else
                        echo "Error: Token alignment failed. Check if LINUX_NET_VAULT_TOKEN starts with 'hvs.' (not allowed for custom IDs)."
                    fi
                fi
            fi
