...
# Linux-Net Environment Check Script
...
check_env_file() {
    if [ ! -f ".env" ]; then
        ...
        if [ -f ".env.example" ]; then
            ...
            print_warning "  - LINUX_NET_REDIS__PASSWORD"
            print_warning "  - LINUX_NET_SERVER__API_KEY"
            return 1
        else
...
check_env_variables() {
    ...
    if [ -z "$LINUX_NET_REDIS__PASSWORD" ] || [ "$LINUX_NET_REDIS__PASSWORD" = "change_this_redis_password" ]; then
        missing_vars+=("LINUX_NET_REDIS__PASSWORD")
    fi
    
    if [ -z "$LINUX_NET_SERVER__API_KEY" ] || [ "$LINUX_NET_SERVER__API_KEY" = "change_this_api_key" ]; then
        missing_vars+=("LINUX_NET_SERVER__API_KEY")
    fi
...
generate_secure_values() {
    ...
    cat > .env << ENVEOF
# Linux-Net Environment Configuration
# Generated on $(date)

# ==========================================
# Core Authentication
# ==========================================
LINUX_NET_REDIS__PASSWORD=$redis_password
LINUX_NET_SERVER__API_KEY=$api_key

# ==========================================
# Server Settings
# ==========================================
# LINUX_NET_SERVER__EXTERNAL_URL=http://localhost:9000
TZ=Asia/Shanghai
LINUX_NET_LOG__LEVEL=INFO
...
LINUX_NET_CREDENTIAL__VERIFY=false
LINUX_NET_CREDENTIAL__CACHE_TTL=30
LINUX_NET_VAULT_TOKEN=$vault_token

# ==========================================
# Audit & Compliance (MongoDB)
# ==========================================
LINUX_NET_MONGODB__ENABLED=false
LINUX_NET_MONGODB__URI=mongodb://localhost:27017
LINUX_NET_MONGODB__DATABASE=linux_net
LINUX_NET_MONGODB__COLLECTION=audit_jobs
LINUX_NET_MONGODB__DETACHED_COLLECTION=audit_detached_tasks
ENVEOF
...
main() {
    echo "Linux-Net Environment Check"
    echo "========================="
