...
class VaultKvCredentialProvider(BaseCredentialProvider):
    ...
    def _load_client_config(plugin_cfg) -> VaultKvConfig:
        ...
        if "token" not in dumped and (token := os.getenv("LINUX_NET_VAULT_TOKEN")):
            dumped["token"] = token
        if "role_id" not in dumped and (role_id := os.getenv("LINUX_NET_VAULT_ROLE_ID")):
            dumped["role_id"] = role_id
        if "secret_id" not in dumped and (secret_id := os.getenv("LINUX_NET_VAULT_SECRET_ID")):
    ...
    def _read_secret(self) -> dict[str, Any]:
        ...
        redis_key = f"linux_net:cache:vault:{ns}:{self.cfg.mount}:{self.cfg.ref}:{ver}"
