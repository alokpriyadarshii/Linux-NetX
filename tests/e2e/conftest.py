...
E2E_BASE_ENV: dict[str, str] = {
    "LINUX_NET_CONFIG_FILE": str(E2E_CONFIG),
    "LINUX_NET_SERVER__API_KEY": API_KEY,
    "LINUX_NET_FAKE_REDIS": "0",
    "LINUX_NET_REDIS__HOST": REDIS_CFG.host,
    "LINUX_NET_REDIS__PORT": str(REDIS_CFG.port),
    "LINUX_NET_REDIS__PASSWORD": REDIS_CFG.password,
...
def _reload_e2e_runtime(
    ...
    for key in [k for k in os.environ if k.startswith("LINUX_NET_") or k == "PYTHONPATH"]:
        monkeypatch.delenv(key, raising=False)
    for key, value in env.items():
        if key.startswith("LINUX_NET_") or key == "PYTHONPATH":
...
def e2e_runtime(monkeypatch: pytest.MonkeyPatch, request) -> E2ERuntime | None:
    """Ensure each e2e test reloads Linux-Net with real Redis config."""
...
def _wait_for_node_queue(runtime: E2ERuntime, timeout: float, proc: subprocess.Popen | None) -> str:
    ...
    from linux_net.models import NodeInfo
