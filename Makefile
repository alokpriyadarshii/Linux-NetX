...
help:
	@echo "Linux-Net Common Operations"
	@echo "========================"
	@echo ""
...
logs:
	...
	if [ -z "$$SERVICE" ]; then \
		...
		echo "💡 Tip: Use 'make logs controller' or 'make logs linux_net-controller-1'"; \
...
shell:
	...
	if [ -z "$$SERVICE" ]; then \
		...
		echo "     make shell linux_net-controller-1  # Container name"; \
...
ai-docs:
	...
	@if [ ! -f llms.txt ]; then \
		...
		echo "# Linux-Net AI Guide" > llms.txt; \
	fi
