...
# Linux-Net One-Click Deployment Script

set -e

echo "Linux-Net One-Click Deployment"
echo "============================="
...
unset LINUX_NET_SERVER__API_KEY LINUX_NET_REDIS__PASSWORD LINUX_NET_LOG_LEVEL 2>/dev/null || true
...
if [ "$LINUX_NET_SERVER__API_KEY" = "change_this_api_key" ] || [ -z "$LINUX_NET_SERVER__API_KEY" ]; then
    echo "LINUX_NET_SERVER__API_KEY is not properly set"
    echo "Current value: $LINUX_NET_SERVER__API_KEY"
    exit 1
fi

if [ "$LINUX_NET_REDIS__PASSWORD" = "change_this_redis_password" ] || [ -z "$LINUX_NET_REDIS__PASSWORD" ]; then
    echo "LINUX_NET_REDIS__PASSWORD is not properly set"
    echo "Current value: $LINUX_NET_REDIS__PASSWORD"
    exit 1
fi

echo "Environment variables loaded correctly:"
echo "  API Key: ${LINUX_NET_SERVER__API_KEY:0:20}..."
echo "  Redis Password: ${LINUX_NET_REDIS__PASSWORD:0:10}..."
...
if docker compose exec controller env | grep -q "LINUX_NET_SERVER__API_KEY=np_"; then
    ...
    docker compose exec controller env | grep LINUX_NET
    echo ""
...
if docker compose ps | grep -q "Up"; then
    ...
    echo "API Key: $LINUX_NET_SERVER__API_KEY"
    echo ""
    echo "Test your deployment:"
    echo "curl -H \"X-API-KEY: $LINUX_NET_SERVER__API_KEY\" http://localhost:9000/health"
    echo ""
