...
from linux_net.models.common import CredentialRef, DriverConnectionArgs, DriverName
from linux_net.models.request import ExecutionRequest


def _load_device_module():
    return importlib.import_module("linux_net.routes.device")


def test_credential_disabled_rejects_request(runtime_loader):
    runtime_loader({"LINUX_NET_CREDENTIAL__ENABLED": "false"})
    device_module = _load_device_module()

    req = ExecutionRequest(
        driver=DriverName.NETMIKO,
        connection_args=DriverConnectionArgs(),
        credential=CredentialRef(name="vault_kv", ref="linux_net/device-a", mount="kv"),
    ...
    runtime_loader(
        {
            ...
            "LINUX_NET_CREDENTIAL__TOKEN": "dev-root-token",
            "LINUX_NET_CREDENTIAL__ALLOWED_PATHS": "kv/allowed",
            "LINUX_NET_CREDENTIAL__CACHE_TTL": "0",
        }
    )

    device_module = _load_device_module()
    from linux_net.plugins.credentials import vault_kv
...
def test_vault_provider_resolves_and_populates(runtime_loader, monkeypatch):
    ...
    secret_store = {"kv/linux_net/device-a": {"username": "admin", "password": "s3cr3t"}}
    ...
    from linux_net.services.rediz import g_rdb
    ...
    assert read_calls == [("kv", "linux_net/device-a", None)]
...
def test_vault_provider_respects_cache(runtime_loader, monkeypatch):
    runtime_loader(
        {
            ...
            "LINUX_NET_CREDENTIAL__TOKEN": "dev-root-token",
            "LINUX_NET_CREDENTIAL__ALLOWED_PATHS": "kv/linux_net",
            "LINUX_NET_CREDENTIAL__CACHE_TTL": "60",
        }
    )
    ...
    secret_store = {"kv/linux_net/device-a": {"username": "cached", "password": "once"}}
    read_calls: list[tuple[str, str, int | None]] = []
...
def test_vault_provider_reads_version_and_mapping(runtime_loader, monkeypatch):
    ...
    secret_store = {"kv/linux_net/device-a": {"user_field": "u1", "pwd_field": "p1"}}
    read_calls: list[tuple[str, str, int | None]] = []
    ...
    assert read_calls == [("kv", "linux_net/device-a", 2)]
...
def test_vault_provider_missing_required_field(runtime_loader, monkeypatch):
    ...
    secret_store = {"kv/linux_net/device-a": {"user_field": "u1"}}
